...
describe('전수 글자 xor 리포트', () => {
  it.skipIf(!exportSource)('앱 기본 획·기본 레이아웃으로 11,172자를 그려 Noto 대비 xor를 낸다', () => {
    ...
    const characters: Record<string, { xor: number; ink: number } | { error: string }> = {}
    const started = Date.now()
    for (const glyph of data.glyphs) {
      const result = glyphXorAgainstNoto({ char: glyph.identity.character, outline: glyph.outline, jamoMaps: JAMO_MAPS, schemas: DEFAULT_LAYOUT_SCHEMAS, globalStyle: DEFAULT_STYLE })
      characters[glyph.identity.character] = result.ok ? { xor: Number(result.value.xorRatio.toFixed(4)), ink: Number(result.value.inkRatio.toFixed(4)) } : { error: result.message }
    }
    ...
    const summary = {
      ...
      source: 'app-default-strokes+default-layout-schemas+default-global-style',
      meaning: '앱 기본 획·기본 레이아웃·기본 전역 스타일로 그린 잉크와 Noto 글자 윤곽의 xor 면적 / Noto 면적. 사용자 편집은 반영 안 됨. 게이트 아님.',
      count: Object.keys(characters).length, okCount: values.length,
    ...
    console.log(`[glyph xor] ${values.length}/${summary.count} ok · median ${(summary.xor.median! * 100).toFixed(1)}% · p95 ${(summary.xor.p95! * 100).toFixed(1)}% · ${summary.elapsedSeconds}s`)
