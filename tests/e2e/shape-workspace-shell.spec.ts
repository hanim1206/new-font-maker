...
test('J-02 전체 4×4 점유 면은 연속 채우기·비우기 draft를 한 transaction으로 저장·취소·복원한다', async ({ page }) => {
  ...
  const secondRowBox = await cells.nth(4).boundingBox()
  const thirdRowBox = await cells.nth(8).boundingBox()
  if (!firstBox || !secondBox || !secondRowBox || !thirdRowBox) throw new Error('면 원자 셀 위치를 찾을 수 없습니다.')
  ...
  await page.mouse.down()
  await expect(precisionDrawer).toContainText('채우는 중')
  ...
  await page.mouse.move(center(firstBox).x, center(firstBox).y)
  await page.mouse.down()
  await page.mouse.move(center(secondRowBox).x, center(secondRowBox).y, { steps: 3 })
  await page.mouse.move(center(firstBox).x, center(firstBox).y, { steps: 3 })
  await page.mouse.move(center(thirdRowBox).x, center(thirdRowBox).y, { steps: 3 })
  expect(await page.evaluate((key) => localStorage.getItem(key), SHAPE_KEY)).toBe(initializedRaw)
  await page.mouse.up()
  await page.waitForTimeout(350)
  ...
  await page.mouse.down()
  await expect(precisionDrawer).toContainText('비우는 중')
