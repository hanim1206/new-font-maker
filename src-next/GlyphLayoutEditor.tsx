...
import { newLayoutEntry, type LayoutEntry } from './layoutEntry'
...
function GlyphLayoutBody({ glyph, initialPart, onCommitted, onEditStrokes, onPickCharacter, onScopeApplied, entry }: { glyph: NotoPresetGlyph; entry?: LayoutEntry; initialPart?: Part; onCommitted?: GlyphLayoutEditorProps['onCommitted']; onEditStrokes?: GlyphLayoutEditorProps['onEditStrokes']; onPickCharacter?: GlyphLayoutEditorProps['onPickCharacter']; onScopeApplied?: GlyphLayoutEditorProps['onScopeApplied'] }) {
  ...
  // 들어올 때 자리. 처음 본 rail은 그때 `original`을 적어 두고, 저장돼 `original`이 바뀌어도 이 값은 그대로다(부모가 글자 · 모드마다 새 것을 준다 — 되돌리기로 다시 열려도 같은 것).
  ...
  // 세션 편집을 비운다. 걸림 표지(`snapHit`)는 남긴다 — 놓는 순간 저장돼도 그 자리에 걸려 있다는 표시는 다음 조작까지 보인다.
  const resetRails = () => { setSlotDeltaByPart([]); setRailDeltaByPart([]); setFacesByPart([]); setFixedRails(new Set()); setError('') }
  // 상시 저장. 보선을 놓는 순간(끌기 끝 · 방향키 한 번) 켠 옵션에 Δ를 저장한다 — 저장 버튼 · 복원 · 떠날 때 묻기가 없다.
  // 되돌리기는 셸 머리의 undo(기록 한 줄 = 놓기 한 번). Δ가 0으로 돌아온 편집은 저장할 게 없어 세션만 비운다.
  useEffect(() => {
    if (dragging || editCount === 0) return
    if (canApply) cardsRef.current?.apply()
    else resetRails()
  }, [dragging, editCount, canApply, propagationEdit])
  ...
  return <div className={styles.editor} data-testid="glyph-layout-editor">
      ...
      {/* 저장하면 Δ가 저장소로 가고 context가 새 original로 다시 풀리므로 세션 편집은 비운다. */}
      <ReviewPropagationCards ref={cardsRef} source={glyph.identity} edit={propagationEdit} changed={changedRails} fixed={fixedRails} focus={rail?.part} railRole={rail?.role} onApplied={resetRails} onCommitted={onCommitted} onSelectPart={selectPart} onScopeChange={setSelection} onScopeApplied={markApplied} />
  </div>
}
...
export interface GlyphLayoutEditorProps {
  ...
  /** 켠 부품의 획 편집으로 내려간다. 주면 켜진 상자를 한 번 더 누르면 간다(누르거나 올리면 상자가 진해진다). 버튼은 없다. */
  ...
  /** 들어올 때 보선 자리와 손댄 rail. 주황 띠 · Δ 숫자의 기준. 호출자가 글자 · 모드마다 새 것을 주면 되돌리기로 다시 열려도 기준이 남는다. */
  entry?: LayoutEntry
}

export function GlyphLayoutEditor({ codepoint, initialPart, onCommitted, onEditStrokes, onPickCharacter, onScopeApplied, entry }: GlyphLayoutEditorProps) {
  const { glyph, error } = useNotoGlyph(codepoint)
  if (glyph) return <GlyphLayoutBody key={codepoint} glyph={glyph} initialPart={initialPart} onCommitted={onCommitted} onEditStrokes={onEditStrokes} onPickCharacter={onPickCharacter} onScopeApplied={onScopeApplied} entry={entry} />
