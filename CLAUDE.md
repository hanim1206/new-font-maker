...
- Obsidian 기획 문서는 짧고 자연스러운 한국어로 핵심만 기록한다.
- Obsidian에서 사용자가 `==하이라이트==`한 내용은 Codex에게 답하거나 검토를 요청한 질문 표지로 우선 해석한다.
- Font Guide Lab의 가이드선 묶음은 `기준선`, 개별 선은 `보선`으로 부른다.
- AI용 줄 단위 형식, 장황한 설명, 딱딱한 템플릿과 과도한 표를 피한다.
- 빠르게 훑어도 방향과 결정이 보이도록 쓴다.
- 제품 논의는 `입력 → 생성 엔진 ↔ 편집 → 출력` 네 영역으로 정리한다.
- 버그, 성능, 리팩터링은 별도 대분류가 아니라 해당 영역의 작업 성격으로 표시한다.
- 어느 영역에도 속하지 않는 전역 이슈만 `공통 기반`으로 둔다.
- 기획 회의와 아이디어 논의 중에는 코드나 Obsidian 문서를 수정하지 않는다.
- 사용자가 최종안을 명확히 승인한 뒤 `문서화해`라고 요청한 경우에만 Obsidian에 기록하고, `구현해`라고 요청한 경우에만 코드를 변경한다. 문서화 승인과 구현 승인은 서로 대신하지 않는다.
- 새 아이디어는 새 파일로 만들고, 기존 아이디어의 발전 내용은 해당 파일에 합친다.
- 회의 원문이 아니라 아이디어의 핵심만 짧게 기록한다.
- 아이디어가 충분히 모이면 함께 검토해 `프로젝트 반영 · 보류 · 제외`로 나눈다.
- 아이디어 폴더는 `/Users/hanim/Documents/Obsidian Vault/Codex/한글 폰트 메이커/아이디어/`이며 파일명은 `YYYY-MM-DD_제목.md`로 한다.
