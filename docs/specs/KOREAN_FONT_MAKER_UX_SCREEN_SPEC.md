...
- STANDALONE/CH/JU/JU_H/JU_V/JO 역할별 X/Y축 코어 Rail 5개. 코어 Rail만 있을 때 기본 원자 셀은 4×4다.
...
- 중성 탭 안의 필터: 세로형 / 가로형 / 혼합형. 섞임홀자는 하나의 자소 카드로 보이며 편집기에 들어간 뒤 `가로 부분 / 세로 부분`을 전환한다.
...
자소 형태 그리드는 결과 뒤에 보이는 안내선이 아니라 선과 면을 만드는 생성 문법이다. 면은 그리드 셀의 점유 집합으로 만들고, 신규 중심선은 Rail 교점과 연결 경로에 고정한다.
...
- 도구 도크:
  ...
  - 곡률·사선.
  - 그리드에 연결.
- 최대 7개 자동 결과 미리보기 스트립. 현재 역할이 쓰이는 카드는 영향 대상으로, 다른 역할 카드는 비교용 읽기 전용으로 표시한다.
- 수치 조절이 필요한 기준선·중심선 선택에만 정밀 조정 드로어를 표시한다.
- 레이아웃 슬롯은 잠긴 외곽선으로만 보이며 이 화면에서는 슬롯 레일을 편집할 수 없다.
- 면 채우기 도구가 활성화되면 현재 역할 part grid의 모든 원자 셀 경계를 표시한다. 중심선과 겹치거나 시스템이 안전하다고 추측한 일부 셀만 추천 대상으로 제한하지 않는다.
- 면 채우기는 캔버스에서 직접 끝나는 조작이므로 별도 정밀 조정 드로어를 표시하지 않는다.
- 중심선 도구가 활성화되면 선택 가능한 Rail 교점과 연결 경로를 표시하며 새 앵커를 자유 좌표에 만들지 않는다.
- 섞임홀자 원형은 같은 화면 안에서 `가로 부분 / 세로 부분`을 전환하되 최종 합성 결과를 계속 함께 보여준다.
...
1. 중심선 도구 선택.
2. 레일 교점에 첫 앵커를 추가하고 Rail 또는 다음 교점으로 경로를 잇는다.
3. 꺾임과 끝점도 Rail 교점에 고정한다. 필요한 위치가 없을 때만 보조선을 먼저 추가한다.
4. 정밀 조절 패드로 선택점·tension·굵기 조정.
5. 드래그 중에는 결과 윤곽만 미리 보고 pointerup에서 한 번 commit한다.

면 추가:

1. 면 채우기 선택.
2. 현재 part grid의 모든 원자 셀을 편집 가능한 바둑판으로 표시한다.
3. 첫 셀이 비어 있으면 이번 제스처는 `채우기`, 차 있으면 `비우기`로 고정한다.
4. 탭하거나 드래그해 지나간 셀에 같은 동작을 적용한다. 같은 셀을 왕복해도 한 번만 처리한다.
5. 드래그 중에는 하나의 면 요소가 소유할 점유 집합과 outer/hole 결과만 미리 본다.
6. pointerup에서 방문한 셀 전체를 한 transaction으로 commit한다. pointercancel·lostpointercapture는 시작 상태로 돌아가며 저장과 이력을 만들지 않는다.
7. 이후 필요한 모서리에 곡률 또는 사선을 별도 제스처로 지정한다.

면 채우기에서 개별 셀을 선택해 다른 칸으로 옮기는 동작은 제공하지 않는다. 하나의 `GridAreaElement`가 여러 셀을 소유하고 사용자는 그 점유 상태를 추가·제거한다.
...
1. 초성 ㄱ 화면에서는 CH 원형만, 단독 ㄱ 화면에서는 STANDALONE 원형만 기본 편집한다.
2. 다른 역할의 모양이 필요하면 `이 역할에서 복사`를 사용자가 명시적으로 실행한다.
3. 역할 간 live link는 별도 승인 전까지 제공하지 않으며 두 역할을 한 제스처로 암묵적으로 함께 수정하지 않는다.
...
- 셀은 master·channel·element와 네 경계 Rail ID로 식별하며 배열 순서나 화면 좌표를 저장 주소로 사용하지 않는다.
- 수정 중 현재 역할이 쓰이는 결과만 갱신하고 다른 역할 카드는 비교용 원본을 유지한다. 현재 보이는 카드부터 계산한다.
...
17. 면 채우기 도구는 현재 part grid의 전체 원자 셀을 직접 조작하며 한 면 요소에 다중 점유를 저장한다.
18. 채우기 드래그는 첫 셀에서 채우기·비우기 방향을 고정하고 pointerup 한 transaction, cancel 무변, Undo 한 단계로 동작한다.
19. 신규 중심선 앵커와 꺾임은 Rail 교점에 고정되며 Rail 이동 후에도 같은 ID 참조를 유지한다.
20. 초성·단독·중성·종성 역할 마스터는 사용자의 명시적 복사·연결 없이 함께 수정되지 않는다.
