...
describe('노토 굵기 곡선', () => {
  ...
  it('굵기가 오르면 배율이 단조롭게 오르고, 900은 앱 배율 2.2보다 얇다', () => {
    ...
    expect(notoThicknessMultiplier(900)).toBeLessThan(weightToMultiplier(900))
    expect(appThicknessErrorAt(900)).toBeGreaterThan(0.1)
    // 100은 앱과 거의 같다(0.4 근처).
    expect(Math.abs(appThicknessErrorAt(100))).toBeLessThan(0.05)
  })
