...
function validateStrokeStyle(value: unknown, path: string, issues: FontPayloadValidationIssue[], allowLegacy: boolean): void {
  ...
  exactKeys(value, value.mode === 'brush' ? [...allowed, 'roundness', 'innerRoundness', 'contrast'] : allowed, allowed, path, issues)
  ...
  if (value.mode === 'brush' && value.innerRoundness !== undefined) finite(value.innerRoundness, `${path}.innerRoundness`, issues)
  if (value.mode === 'brush' && value.contrast !== undefined) finite(value.contrast, `${path}.contrast`, issues)
