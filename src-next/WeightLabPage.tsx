...
function LabGlyph({ char, weight, mode, ghostPath, showGhost }: { char: string; weight: number; mode: Mode; ghostPath: string | undefined; showGhost: boolean }) {
  ...
  const multiplier = mode === 'naive' ? legacyWeightToMultiplier(weight) : notoThicknessMultiplier(weight)
    ...
    <main className={styles.page}>
      <header className={styles.header}>
        <div>
          ...
          <h1>굵기 보정 실험실</h1>
          <p>같은 글자를 세 칸에 놓고 굵기를 움직인다. 왼쪽은 09-26 이전 직선 배율, 가운데는 노토 굵기 곡선(지금 앱)에 상자 밀기를 얹은 것, 오른쪽은 노토 실제 윤곽. 저장하지 않는다.</p>
        </div>
      ...
      <section className={styles.panels}>
        <article className={styles.panel}>
          <h2>옛 직선 <small>09-26 이전 앱 · 배율 {legacyMultiplier.toFixed(2)}</small></h2>
          <LabGlyph char={char} weight={weight} mode="naive" ghostPath={ghost?.path} showGhost={showGhost} />
          <p>중심선은 그대로, 두께만 100→0.4 · 400→1 · 900→2.2 직선으로 곱하던 것. 지금 제품은 가운데 칸의 곡선을 쓴다.</p>
        </article>
        <article className={styles.panel}>
          <h2>노토 곡선 보정 <small>지금 앱 · 배율 {notoVertical.toFixed(2)} · 상자 {inset >= 0 ? '−' : '+'}{Math.abs(inset * 1000).toFixed(1)}u</small></h2>
      ...
      <section className={styles.numbers} aria-label="측정값">
        ...
        <table>
          <tbody>
            <tr><th>세로줄기 두께 배율</th><td data-testid="weight-lab-noto-vertical">{ratio(row?.verticalThicknessRatio)}</td><th>옛 직선 배율</th><td>{legacyMultiplier.toFixed(2)}</td><th>옛 직선이 더 두꺼웠던 정도</th><td data-testid="weight-lab-error">{percent(error)}</td></tr>
