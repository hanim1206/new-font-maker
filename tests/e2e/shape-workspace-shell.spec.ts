...
test('J-02 전체 4×4 점유 면은 연속 채우기·비우기 draft를 한 transaction으로 저장·취소·복원한다', async ({ page }) => {
  ...
  const affectedContexts = page.getByRole('list', { name: '면이 반영되는 초성 조합 6개' })
  await expect(affectedContexts.getByRole('listitem')).toHaveCount(6)
  await expect(page.getByRole('button', { name: 'ㄱ 단독 관찰' })).toHaveCount(0)
  await expect(page.getByText('단독 ㄱ은 별도 STANDALONE 원형이므로 이 비교에서 제외합니다.')).toBeVisible()
  await expect(page.getByRole('region', { name: '정밀 조절' })).toHaveCount(0)
  ...
  if (!firstBox || !secondBox) throw new Error('면 원자 셀 위치를 찾을 수 없습니다.')
  ...
  await expect(second).toHaveAttribute('data-draft', 'true')
  ...
  const fastStartBox = await cells.nth(12).boundingBox()
  const fastEndBox = await cells.nth(15).boundingBox()
  if (!fastStartBox || !fastEndBox) throw new Error('빠른 가로 드래그 셀 위치를 찾을 수 없습니다.')
  await page.mouse.move(center(fastStartBox).x, center(fastStartBox).y)
  await page.mouse.down()
  await page.mouse.move(center(fastEndBox).x, center(fastEndBox).y)
  await page.mouse.up()
  await page.waitForTimeout(350)
  await expect(page.locator('[data-area-cell][data-occupied="true"]')).toHaveCount(4)
  await page.getByRole('button', { name: '형태 편집 실행 취소' }).click()
  await page.waitForTimeout(350)
  expect(await page.evaluate((key) => localStorage.getItem(key), SHAPE_KEY)).toBe(initializedRaw)
  await page.evaluate(() => { (window as unknown as { __shapeAreaWrites: number }).__shapeAreaWrites = 0 })

  const fillFirstBox = await cells.nth(0).boundingBox()
  const fillSecondRowBox = await cells.nth(4).boundingBox()
  const fillThirdRowBox = await cells.nth(8).boundingBox()
  if (!fillFirstBox || !fillSecondRowBox || !fillThirdRowBox) throw new Error('세로 드래그 셀 위치를 찾을 수 없습니다.')
  await page.mouse.move(center(fillFirstBox).x, center(fillFirstBox).y)
  await page.mouse.down()
  await page.mouse.move(center(fillSecondRowBox).x, center(fillSecondRowBox).y, { steps: 3 })
  await page.mouse.move(center(fillFirstBox).x, center(fillFirstBox).y, { steps: 3 })
  await page.mouse.move(center(fillThirdRowBox).x, center(fillThirdRowBox).y, { steps: 3 })
  expect(await page.evaluate((key) => localStorage.getItem(key), SHAPE_KEY)).toBe(initializedRaw)
  await page.mouse.up()
  await page.waitForTimeout(350)
  ...
  const filledSecond = page.getByRole('button', { name: '면 셀 2행 1열, 찬 셀' })
  const eraseFirstBox = await cells.nth(0).boundingBox()
  const eraseSecondBox = await cells.nth(4).boundingBox()
  ...
  await page.mouse.down()
  await expect(cells.nth(0)).toHaveAttribute('data-draft', 'true')
  ...
  await expect(page.locator('[data-area-cell][data-occupied="true"]')).toHaveCount(1)
  ...
  await expect(page.getByRole('region', { name: '정밀 조절' })).toBeVisible()
  await expect(page.getByRole('slider', { name: /캔버스 안쪽 .* 기준선/ })).toHaveCount(4)
  await expectMobileShellContract(page)
})
