...
> 상태: `pending approval`
> 개정일: 2026-08-23
> 이번 개정의 핵심: 네모틀 한글을 위한 문맥별 자소 베리에이션, 5×5 코어 그리드, 구현 전 결정·검증 게이트
...
1. **5×5 코어 레일 충분성:** `ㄱ·ㅇ·ㅏ·ㅂ·ㅎ·ㅙ`에서 보조 레일 수와 편집성을 측정한다. 대부분의 자소가 처음부터 다수의 보조 레일을 요구하면 코어 의미를 재검토한다.
...
- 하나의 `jamoId + role` 기본 마스터는 레이아웃 문맥에 맞춰 자동 파생된다. 사용자는 7개 문맥 결과를 함께 확인하고 필요한 문맥만 보정할 수 있다.
- 문맥별 기본 파생은 역할·문맥 프리셋으로 모든 관련 자소에 일괄 적용되며, 특정 자소의 보정은 그 위에 override로 저장한다.
- 자소 형태 그리드는 최소 5×5 의미 기반 코어 레일을 공통 계약으로 가진다. 복잡한 자소에는 코어 레일 사이에 보조 레일을 추가할 수 있다.
...
- 코어 레일은 형태를 5×5 칸에 강제로 가두기 위한 격자가 아니라 프리셋·베리에이션·초기화가 공유하는 좌표 문법이다.
...
- 모든 역할별 기본 형태 그리드가 같은 의미의 5×5 코어 레일을 가진다.
...
- 같은 `ㄱ` 기본 마스터가 7개 문맥 프리셋을 통해 서로 다른 슬롯 비례로 자동 파생된다.
...
9. 큰 캔버스에 `레이아웃·코어 레일·보조 레일·선·면·곡률·사선` 도구와 `결과 윤곽·원본 선·점유 면·레이아웃 영역` 레이어를 제공한다.
10. 편집 모드를 명확히 분리한다.
...
15. 첫 수직 검증은 `ㄱ` 기본 마스터와 가로중성 문맥 보정, `ㅇ` 면+내부 공간과 납작형 문맥 보정, `ㅏ` 기존 중심선, `ㅂ·ㅎ·ㅙ`의 5×5 충분성 검증으로 제한한다.
16. 굵기 UI를 조건부로 변경한다.
   - 선 전용: `전체 획 굵기`.
   - 혼합: `선 획 굵기`.
   - 면 전용: 숨김.
...
- `src/services/jamoContextVariantCommands.ts`
...
- 채우기 드래그, 레일 드래그, 참조 재연결 한 번이 각각 Undo 한 단계다.
- 전역 weight 변경이 면 좌표나 점유 상태를 바꾸지 않는다.
- `ㄱ·ㅇ·ㅏ·ㅂ·ㅎ·ㅙ` 검증 결과로 5×5 코어 계약의 유지 또는 재검토가 기록된다.
...
15. Step 3 이후 여러 store를 바꾸는 command도 하나의 transaction으로 Undo/Redo된다.
16. 화면과 OTF가 같은 `resolvedPartGrid`, `ResolvedInkPrimitive[]`, union 규칙을 사용한다.
17. 선 전용 전체 굵기, 혼합형 선 굵기, 면 전용 굵기 숨김이 정확히 적용된다.
18. pointerup만 편집을 커밋하고 pointercancel은 원본으로 롤백한다.
19. 프로젝트 저장·불러오기·재접속·모바일·OTF가 같은 원본 데이터를 사용한다.
20. 대표 수직 검증을 통과하기 전에는 67개 자모 자동 변환을 시작하지 않는다.
...
- 사용자는 7개 파생 결과를 확인하고 필요한 문맥만 수정할 수 있다.
