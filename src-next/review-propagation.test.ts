...
describe('propagationCandidates', () => {
  ...
  const charactersOf = (input: Parameters<typeof propagationCandidates>[0]) => propagationCandidates(input).map((item) => item.character).join('')
  it('이 레이아웃은 고른 글자 묶음이 먼저 나오고, 다 돌면 기계 조합으로 넘어간다', () => {
    const 간 = corpusIdentity('간'.codePointAt(0)!)
    expect(charactersOf({ source: 간, scope: 'layer', count: 8 })).toBe('한임명집실것없력')
    expect(charactersOf({ source: 간, scope: 'layer', count: 8, page: 2 })).toBe('일많겠성힘닭짧적')
    const after = propagationCandidates({ source: 간, scope: 'layer', count: 8, page: 3 })
    expect(after).toHaveLength(8)
    expect(after.every((item) => item.contextId === 'right-final' && !'한임명집실것없력있심년할책정법같일많겠성힘닭짧적'.includes(item.character))).toBe(true)
  })
  it('원본이 고른 묶음에 있으면 빠지고 같은 레이아웃의 다른 고른 글자가 메운다', () => {
    const picked = propagationCandidates({ source: corpusIdentity('한'.codePointAt(0)!), scope: 'layer', count: 8 })
    expect(picked).toHaveLength(8)
    expect(picked.map((item) => item.character)).not.toContain('한')
    expect(picked.slice(0, 7).map((item) => item.character).join('')).toBe('임명집실것없력')
  })
  it('전체는 고른 글자를 여섯 레이아웃에서 돌아가며 뽑고 묶음끼리 안 겹친다', () => {
    const 간 = corpusIdentity('간'.codePointAt(0)!)
    const pages = [0, 1, 2].map((page) => propagationCandidates({ source: 간, scope: 'all', count: 8, page }))
    expect(new Set(pages[0].map((item) => item.contextId)).size).toBe(6)
    expect(new Set(pages[0].map((item) => item.initialJamo)).size).toBe(8)
    expect(new Set(pages.flat().map((item) => item.codepoint)).size).toBe(24)
  })
  it('이 자모만은 고른 글자가 앞에 서고 모자란 자리를 기계 조합이 메운다', () => {
    const 간 = corpusIdentity('간'.codePointAt(0)!)
    const first = propagationCandidates({ source: 간, scope: 'jamo', count: 8, focus: 'CH', jamos: ['ㅎ'] })
